...
def main():
    ...
    if args.data_path and args.data_path.exists():
        ...
        df = simulate_satellite_data(config['data']['n_points'], config['data']['n_bands'], config['data']['seed'])
    ...
    if config['analysis']['calculate_ndvi'] and config['analysis']['red_band'] in df.columns and (config['analysis']['nir_band'] in df.columns):
        calculate_ndvi(df, config['analysis']['red_band'], config['analysis']['nir_band'])
    logging.info(f'Mean NDVI: {ndvi.mean():.4f}')
    logging.info(f'NDVI Range: [{ndvi.min():.4f}, {ndvi.max():.4f}]')
    plot_earth_observation(df, band_cols, 'Remote Sensing Earth Observation', output_dir / 'earth_observation.png')
